...
set.asPOSIXct <- function(inFrame) { 
  dteday <- as.POSIXct(as.integer(inFrame$dteday), origin = "1970-01-01")
  as.POSIXct(strptime(paste(as.character(dteday), " ", as.character(inFrame$hr), ":00:00", 
                            sep = ""), 
                      "%Y-%m-%d %H:%M:%S"))
}
...
char.toPOSIXct <- function(inFrame) {
  as.POSIXct(strptime(paste(inFrame$dteday, " ", as.character(inFrame$hr), ":00:00", sep = ""), 
                      "%Y-%m-%d %H:%M:%S")) 
}
...
if(Azure) {
  ...
  BikeShare <- read.csv("BikeSharing.csv", sep = ",", header = TRUE, stringsAsFactors = FALSE)
...
BikeShare$isWorking <- with(BikeShare, ifelse(workingday & !holiday, 1, 0))
...
BikeShare$workTime <- with(BikeShare, ifelse(isWorking, hr, hr + 24))

## Shift the order of the hour variable so that it is smoothly
## "humped over 24 hours.
BikeShare$xformHr <- with(BikeShare, ifelse(hr > 4, hr - 5, hr + 19))

## Add a variable with unique values for time of day for working and non-working days.
BikeShare$xformWorkHr <- with(BikeShare, ifelse(isWorking, xformHr, xformHr + 24)) 
...
We’ll use lm() to compute a linear model used for de-trending the response variable column in the data frame. De-trending removes a source of bias in the correlation estimates. We are particularly interested in the correlation of the predictor variables with this detrended response.
...
xAxis <- list("hr", "xformWorkHr","mnth", "weathersit", "isWorking", "dayWeek")
...
Map(function(time, label) { 
  ggplot(BikeShare[BikeShare$hr == time, ], aes(x = isWorking, y = cnt, group = isWorking)) + 
    geom_boxplot() + 
    ggtitle(label) +
    theme(text = element_text(size = 18))},
  Times, labels)
...
```{r comment=NA}
## Add a variable with unique values for time of day for working and non-working days.
BikeShare$workTime <- with(BikeShare, ifelse(isWorking, hr, hr + 24)) # replaced by 'xformWorkHr'?
```
...
## Shift the order of the hour variable so that it is smoothly 
## "humped over 24 hours. 
#BikeShare$xformHr <- with(BikeShare, ifelse(hr > 4, hr - 5, hr + 19)) # already created
```

We can add one more plot type to the scatter plots we created in the visualization model, with the following code:

```{r comment=NA}
# from "visualize.R" - already plotted, but modified to just create Figure 17

## Look at the relationship between predictors and bike demand
labels <- c("Bike demand vs xformHr")
xAxis <- c("xformHr")
Map(function(X, label){ 
  ggplot(BikeShare, aes_string(x = X, y = "cnt")) + 
    geom_point(aes_string(colour = "cnt"), alpha = 0.1) + 
...
## Some utility functions - already defined?
#set.asPOSIXct <- function(inFrame) { 
#  dteday <- as.POSIXct(as.integer(inFrame$dteday), origin = "1970-01-01")
#  as.POSIXct(strptime(paste(as.character(dteday), " ", as.character(inFrame$hr), ":00:00", 
#                            sep = ""), "%Y-%m-%d %H:%M:%S"))
...
#} else{
#  dataset <- read.csv("BikeSharing.csv", sep = ",", header = T, stringsAsFactors = F )
#  dataset$dteday <- char.toPOSIXct(dataset)
#}

dataset <- BikeShare # added
#require(randomForest) # original
library(randomForest) # edited
...
## Some utility functions - already loaded???
#set.asPOSIXct <- function(inFrame) { 
#  dteday <- as.POSIXct(
...
#require(randomForest) # already loaded
...
names(inFrame) <- c("predicted", "cnt", "dteday", "monthCount", "hr", "workTime")
...
lapply(times, function(times){
  ggplot() +
    geom_line(data = inFrame[inFrame$hr == times, ], aes(x = dteday, y = cnt)) +
    geom_line(data = inFrame[inFrame$hr == times, ], aes(x = dteday, y = predicted), 
              color = "red") +
    ylab("Log number of bikes") +
    labs(title = paste("Bike demand at ", as.character(times), ":00", spe ="")) +
    theme(text = element_text(size=20))
...
inFrame <- mutate(inFrame, fact.hr = as.factor(hr), fact.workTime = as.factor(workTime))                                  
...
Quantile <- 0.10
#require(dplyr) # already loaded (2X?)
...
#require(randomForest) # original
#library(randomForest) # edited - already loaded
rf.bike <- randomForest(cnt ~ xformWorkHr + dteday + temp + hum, 
                        data = BikeShare, 
                        ntree = 40, 
                        importance = TRUE, 
                        nodesize = 5)

# https://github.com/Quantia-Analytics/Azure-ML-Bike-Demand-Forecast/blob/master/utilities.r
...
#require(nnet)
library(gam)
#library(randomForest) # already loaded
library(kernlab)
library(nnet)
