...
Figure 2.17: Run Times for Multiple Races. These line plots show the times for male runners who completed at least 8 Cherry Blossom races. Each set of connected segments correpsonds to the run times for one athlete. Looking at all line plots, we see a similar shape to the scatter plot in Figure 2.7, i.e., an upward curve with age. However, we can also see how an individual's preformance changes. For example, many middle-aged runners show a sharp increase in run time with age but aht si nto the case for all. Some of them improve and others change more slowly.
